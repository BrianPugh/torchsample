...
def gamma(coords, order=10):
    ...
    for o in range(order):
        freq = (2**o) * pi
        cos = jt.cos(freq * coords)
        sin = jt.sin(freq * coords)
        output.append(cos)
        output.append(sin)

    output = jt.concat(output, dim=-1)
...
def nearest_pixel(coords, size, align_corners=default.align_corners):
    ...
    unnorm_offset = 2 * (jt.round(unnorm_coords) - unnorm_coords)
    output = jt.concat((coords, unnorm_offset), dim=-1)
    return output
